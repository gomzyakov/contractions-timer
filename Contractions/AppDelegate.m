...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    // connect to contractions model
    _contractions = [[Contractions alloc] init];
    [_contractions loadContractions];

    self.window.rootViewController = [self rootNavigationController];

    [self __customizeAppearance];

    [self.window makeKeyAndVisible];
    return YES;
}

- (void)__customizeAppearance
{
    const CGFloat kTitleFontSize = 18.0;
...
- (UINavigationController *)rootNavigationController
{
    ...
    UINavigationController *mainNavigationController = [[UINavigationController alloc] initWithRootViewController:timerViewController];
...
- (NSManagedObjectModel *)managedObjectModel
{
    ...
    NSURL *modelURL = [[NSBundle mainBundle] URLForResource:@"contractions" withExtension:@"momd"];
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator
{
    ...
    NSURL *storeURL = [[self applicationDocumentsDirectory] URLByAppendingPathComponent:@"contractions.sqlite"];
